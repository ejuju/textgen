...
import (
	...
	"github.com/ejuju/textgen/pkg/markov"
)

func main() {
	markovChain := markov.NewChain(-1, rand.NewSource(0))
	markovChain.Learn(corpus)

	chars := "I have "
	for {
		nextChar := markovChain.NextCharacter(chars)
		chars += string(nextChar)
